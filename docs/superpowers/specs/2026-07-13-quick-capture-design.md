...
- **レスポンス**: `201 { task, parsed: { title, projectName, dueDate, priority }, summary }`。`summary` はサーバーが組み立てた通知用1行文字列（例: `Sales: 見積送付 (due 2026-07-18, high)`）。クライアントはJSONを深くパースせず summary をそのまま通知に表示できる
